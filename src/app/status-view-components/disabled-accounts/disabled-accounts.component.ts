...
@Component({
  selector: 'app-disabled-accounts',
  templateUrl: './disabled-accounts.component.html',
  styleUrls: ['./disabled-accounts.component.css']
})
export class DisabledAccountsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getDisabledAccounts();
  }

  getDisabledAccounts(): void {
    this.accountService.getAccountsByStatus("CLOSED")
    .subscribe(accounts => {
      this.accounts = accounts;
      this.accountService.getAccountsByStatus("DENIED").subscribe(denied_accounts => {
        denied_accounts.forEach(element => {
